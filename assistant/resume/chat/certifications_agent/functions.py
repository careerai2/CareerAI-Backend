...
async def apply_patches(thread_id: str, patches: list[dict]):
    """
    Applies JSON patches to the entire certification section of the resume.
    Handles creation, replacement, or removal of certifications at any index.
    ...
    try:
        ...
        current_certifications = current_resume.get("certifications", [])
        if not isinstance(current_certifications, list):
            current_certifications = []

        # ✅ Apply patches to entire certifications list
        try:
            jsonpatch.apply_patch(current_certifications, patches, in_place=True)
            print(f"Applied patch list to certifications: {patches}")
        except jsonpatch.JsonPatchException as e:
            return {"status": "error", "message": f"Failed to apply patch list: {e}"}

        # Save updated certifications back to resume
        current_resume["certifications"] = current_certifications
